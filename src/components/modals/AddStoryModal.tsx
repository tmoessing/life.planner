...
import { AddStoriesViewRefactored } from '@/components/views/AddStoriesViewRefactored';
...
export function AddStoryModal({ open, onOpenChange, initialData, targetColumnId }: AddStoryModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      // Apply rules after merging initialData
      const appliedActions = evaluateRules('story-create', mergedData);
  ...
  }, [open, initialData, targetColumnId, evaluateRules]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${isBulkMode ? 'max-w-[95vw] h-[90vh]' : 'max-w-4xl max-h-[90vh]'} w-full overflow-y-auto p-2 sm:p-6 transition-all duration-300`}>
        <DialogHeader className="pb-1 sm:pb-4 flex flex-row items-center justify-between">
          <div>
            <DialogTitle className="text-sm sm:text-xl">{isBulkMode ? 'Bulk Add Stories' : 'Add New Story'}</DialogTitle>
            <DialogDescription className="text-xs sm:text-sm hidden sm:block">
              {isBulkMode ? 'Add multiple stories at once.' : 'Create a new story to track your tasks and goals.'}
            </DialogDescription>
          </div>
          <Button
            variant="outline"
            ...
            className="ml-auto"
          >
            {isBulkMode ? 'Single Story Mode' : 'Switch to Bulk Mode'}
          </Button>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-6">
          {isBulkMode ? (
            <div className="pt-2">
              <AddStoriesViewRefactored onSuccess={() => onOpenChange(false)} />
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-6">
                ...
                <div className="space-y-1.5 sm:space-y-4">
                  ...
                  {/* Role */}
                  <div className="space-y-1 sm:space-y-2">
                    <label className="text-xs sm:text-sm font-medium">Story Role</label>
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select story role..." />
                      </SelectTrigger>
                      <SelectContent>
