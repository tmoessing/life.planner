...
export function GoalKanbanBoard({ goal, onBack }: GoalKanbanBoardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Kanban Board */}
      <BoardGridLayout
        columns={Object.entries(storiesByStatus).map(([status, stories]) => ({
          id: status,
          label: getStatusText(status),
          items: stories,
          color: goalSettings.getStatusColor(status)
        }))}
        renderItem={(story) => (
        ...
        gridClassName="gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6"
      />
